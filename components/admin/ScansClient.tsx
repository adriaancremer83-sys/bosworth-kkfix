...
function exportCSV(scans: Scan[]) {
  const headers = ['Timestamp', 'City', 'Region', 'Country', 'Device', 'Batch', 'Unit', 'IP', 'Lat', 'Lng', 'Location Source']
  const rows = scans.map(s => [
    s.timestamp, s.city ?? '', s.region ?? '', s.country ?? '',
    s.device_type ?? '', s.batch_id ?? '', s.unit_id ?? '',
    s.ip_address ?? '', s.lat ?? '', s.lng ?? '', s.location_source ?? '',
  ])
...
export default function ScansClient({ initialScans }: { initialScans: Scan[] }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      console.error('[scans] refresh error:', err)
    } finally {
      setSyncing(false)
    }
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={refresh} style={{ ...inputStyle, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', border: '1px solid #383838', padding: '8px 14px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '700px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #1e1e1e' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, width: '160px' }}>Timestamp</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>Location</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, width: '100px' }}>Device</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, width: '140px' }}>Batch / Unit</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, width: '130px' }}>IP</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
